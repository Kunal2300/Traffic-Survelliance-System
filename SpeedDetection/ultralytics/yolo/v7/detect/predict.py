...
def numberplateDetection(co,img):
    global bt 
    cv2.imwrite("result/temp1.png",img)
    img=Image.open("result/temp1.png")
    bt+=1
    ...
    # cv2.imwrite("result/1output"+f"{bt}"+".png",gray)

        # first use a kernal
    filter=cv2.bilateralFilter(gray,11,17,17)
    # cv2.imwrite("result/2output"+f"{bt}"+".png",filter) # save filter image

    # get edged image we have to use the canny function from the cv2
    edged=cv2.Canny(filter,30,200)
    # cv2.imwrite("result/3output"+f"{bt}"+".png",edged) #save the edgedcar image
    ...
    try: 
        ...
        cv2.imwrite("result/4output"+f"{bt}"+".png",bbCar)
        ...
        cv2.imwrite("result/5output"+f"{bt}"+".png",plate)
        # extract numberplate
...
def estimatespeed(img,co, Location1, Location2):
...
def draw_boxes(img, bbox, names,object_id, identities=None, offset=(0, 0)):
    ...
    for i, box in enumerate(bbox):
        ...
        if len(data_deque[id]) >= 2:
          ...
          object_speed = estimatespeed(img,co,data_deque[id][1], data_deque[id][0])
        #   print("\n Speed is " , object_speed )
